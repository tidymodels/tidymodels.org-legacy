...
"Multivariate analysis" usually refers to the situation where multiple _outcomes_ are being modeled, analyzed, and/or predicted. There are multivariate versions of many common statistical tools. For example, suppose there was a data set with columns `y1` and `y2` containing two outcomes to be predicted. The `lm()` function would look something like:

```{r lm, eval = FALSE}
lm(cbind(y1, y2) ~ ., data = dat)
```

The `cbind()` call is pretty awkward and is a consequence of how the traditional formula infrastructure works. The recipes package is a lot easier to work with and this article shows an example to illustrate how to model multiple outcomes.   

...
This article requires that you have the following packages installed: kernlab, modeldata, and tidymodels.

Many of the examples for model tuning focus on [grid search](TODO). For that method, all the candidate tuning parameter combinations are defined prior to evaluation. Alternatively, _iterative search_ can be used to analyze the existing tuning parameter results and then _predict_ which tuning parameters to try next. 
...
These two objects will be combined into a single object via the `workflow()` function from the workflows package; this object will be used in the optimization process. 
...
#> i Estimating performance
#> ✓ Estimating performance
#> ⓧ Newest results:	roc_auc=0.769 (+/-0.018)
#> 
#> ── Iteration 23 ──────────────────────────────────────────────────────
#> 
#> i Current best:		roc_auc=0.8728 (@iter 15)
...
#> i Generating 5000 candidates
#> i Predicted candidates
#> i cost=0.162, rbf_sigma=0.934, num_comp=6
#> i Estimating performance
#> ✓ Estimating performance
#> ⓧ Newest results:	roc_auc=0.8621 (+/-0.00645)
#> 
#> ── Iteration 45 ──────────────────────────────────────────────────────
#> 
#> i Current best:		roc_auc=0.8728 (@iter 15)
...
The best performance of the initial set of candidate values was `AUC = 0.865 `. The best results were achieved at iteration 15 with a corresponding AUC value of 0.873. The five best results were:
